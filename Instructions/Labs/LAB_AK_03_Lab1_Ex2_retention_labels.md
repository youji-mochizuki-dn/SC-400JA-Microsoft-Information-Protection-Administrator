...
8. **「保持の設定」** ページの **「アイテムを特定の期間保持する」** 設定を有効にします。
...
18. **「設定の確認」** ページで、**「ラベルの作成」** ボタンを選択します。**「ラベルを保存しました」** を選択して、**「完了」** を選択します。

7 年間の保持期間を持つ VAT 還付用の保持ラベルと、クレジット カードの領収書用の保持ラベルが正常に作成されました。

### タスク 2 – 保持ラベルを発行する

タスク 1 に続いて、VAT 還付の保持ラベルを発行します。これにより、発行されたラベルを財務ユーザーが Exchange メールや SharePoint ドキュメントの場所にあるドキュメントに適用できるようになります。
...
13. **「設定の確認」** ページで、**「送信」** ボタンを選択します。  ポリシーを発行したら、**「完了」** を選択します。

VAT の還付と関連書類の保持ラベルが正常に発行されました。

### タスク 3 – 自動適用の保持ラベルを発行する

タスク 1 に引き続き、クレジット カードの領収書の保持ラベルを自動適用します。 
...
6. **「ラベルの自動適用」** ボタンを選択します。  「コンテンツにラベルを自動で適用する」ウィザードが表示されます。
...
9. **「ラベル付けの対象の情報」** ページでは、「このラベルを適用するコンテンツの種類を選択」で次のオプションを選択します。
...
16. **「場所」** ページで、**「Exchange メール、OneDrive、SharePoint サイト、Microsoft 365 グループ」** オプションを有効にして、「**次へ**」を選択します。

17. **「ラベル」** ページで **「次へ」** を選択します。

18. **「完了」** ページで **「送信」** ボタンを選択します。
...
7.  **「フォルダーのポリシー」** ドロップダウンリストがあれば、**「VAT の還付と関連書類」** に設定します。それ以外の場合は、既存のポリシーから **「5 年で削除」** を選択します (この演習では設定を適用するだけです)。
...
8. 強調表示されたドキュメントに対して、 **...** ボタンを選択します

9. メニューから **「詳細」** ボタンを選択します

10. サイド メニューが右側に表示されます。

11. オプションが利用可能な場合は、**「保持ラベルを適用する」** を **「VAT の還付と関連書類」** に設定します。保持ラベルの発行には時間がかかる場合があるため、すぐにオプションが利用できない場合もあります。利用できない場合は、次のタスクに進みましょう。
...
3. Microsoft 365 が開いたら、左上の角にあるアプリ ランチャー アイコン（9つのドット）を選択し、サブメニューから **OneDrive** を選択します。

4.  OneDrive アプリケーション内で、**Contractor Legal Info.docx** を強調表示します (選択はしません)。

5. 強調表示されたドキュメントに対して、 **...** ボタンを選択します

6. メニューから **「詳細」** ボタンを選択します

7. サイド メニューが右側に表示されます。
